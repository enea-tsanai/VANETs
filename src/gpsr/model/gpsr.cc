...
{
//  std::cout<< "SendPacketFromQueue Node: [" << m_ipv4->GetObject<Node> ()->GetId () << "]\n";
  ...
  if(m_neighbors.isNeighbour (dst))
    ...
    Vector dstVel = m_locationService->GetVelocity (dst);
//    std::cout << "Destination: " << dstPos << "\n";
    nextHop = m_neighbors.BestNeighbor (dstPos, dstVel, myPos, myVel);
//    std::cout << "---\n SendPacketFromQueue Node: [" << m_ipv4->GetObject<Node> ()->GetId () << "] -> NextHop: " << nextHop 
//              << "---\n ";
    if (nextHop == Ipv4Address::GetZero ())
      {
        NS_LOG_LOGIC ("Fallback to recovery-mode. Packets to " << dst);
//        std::cout<< "Fallback to recovery-mode. Packets to " << dst << "\n";
        recovery = true;
      }
    if(recovery)
      {
...
//      NS_LOG_DEBUG ("Received Packet with SRN = " << tag.Get());
  ...
  UpdateRouteToNeighbor (sender, receiver, Position, Velocity, hello_snr);
}


void
RoutingProtocol::UpdateRouteToNeighbor (Ipv4Address sender, Ipv4Address receiver, Vector Pos, Vector Vel, double snr)
{
  m_neighbors.AddEntry (sender, Pos, Vel, snr);
}
...
//  std::cout << "*******\nSendHello-> Node: " << m_ipv4->GetObject<Node> ()->GetId() 
//          << "\npositionX: " << positionX
//          << "\npositionY: " << positionY
//          << "\nvelocityX: " << velocityX
//          << "\nvelocityY: " << velocityY << "\n*******";
...
//   m_neighbors.Purge ();
  ...
  if(m_neighbors.isNeighbour (destination))
  ...
  else
    {
      ...
      Vector dstVel = m_locationService->GetVelocity (destination);
      nextHop = m_neighbors.BestNeighbor (dstPos, dstVel, myPos, myVel);
//      std::cout << "---\n NextHop: " << nextHop << "---\n ";
    }
...
RoutingProtocol::Forwarding (Ptr<const Packet> packet, const Ipv4Header & header,
                             UnicastForwardCallback ucb, ErrorCallback ecb)
{
//  std::cout << "-------\nForwarding| Node: [" << m_ipv4->GetObject<Node> ()->GetId () << "]\n";
  ...
  uint32_t updated = 0;
  Vector Position; // Dst position
  Vector dstVel; // Dst velocity
  Vector RecPosition;
  uint8_t inRec = 0;
  ...
  if (tHeader.Get () == GPSRTYPE_POS)
    {
      ...
      inRec = hdr.GetInRec ();
//      std::cout<< "Forwarding - read header| dstPos: " << Position << " Updated: "
//              << updated << " InRec: " << (int) inRec << " \n";
    }
  ...
  myPos.y = MM->GetPosition ().y; 
  myVel = MM->GetVelocity ();
//  myVel.x = MM->GetVelocity ().x;
//  myVel.y = MM->GetVelocity ().y;
  ...
  if(inRec){
    ...
    RecoveryMode (dst, p, ucb, header);
//    std::cout << "Forwarding| In recovery mode - return..\n";
    return true;
  }
  ...
  if (myUpdated > updated) //check if node has an update to the position of destination
    {      
      ...
      //      DstVelocity.x = m_locationService->GetVelocity (dst).x;
      //      DstVelocity.y = m_locationService->GetVelocity (dst).y;
      updated = myUpdated;     
    }
  ...
  if(m_neighbors.isNeighbour (dst))
    {
      nextHop = dst;
//      if (p->GetSize() > 1000) std::cout << "Forwarding| Next hop is the destination!\n";
    }
  else
    {
//      if (p->GetSize() > 1000)
//      {
//        std::cout << "~~~~~~~~~~~~~~~~~~\nThe mechanism.."
//                  << "Forwarding| dstPos " << Position << " MyPos: " << myPos << "\n";  
//      }
//      if (dstVel.x == 0.0) std::cout << "We have an issue here!!\n";
      nextHop = m_neighbors.BestNeighbor (Position, dstVel, myPos, myVel);
//      if (p->GetSize() > 1000) std::cout << "Forwarding| Next hop is " << nextHop << "\n";
//      std::cout << "---\n F Node: [" << m_ipv4->GetObject<Node> ()->GetId () << "] -> NextHop: " << nextHop 
...
//        std::cout << "\n*Node"<< route->GetOutputDevice ()->GetNode()->GetId() << " forwarding to " << dst << " from " << origin << " through " << route->GetGateway () << " packet " << p->GetUid ();
...
//std::cout << "-------\nRoute Output| Node: [" << m_ipv4->GetObject<Node> ()->GetId () << "]\n";
  if (!p)
    {
...
//  std::cout << "Packet: " << p->GetUid() << " size: "<<  p->GetSize() <<"\n";
  Vector dstPos = Vector (-1, -1, 0);
  Vector dstVel = Vector (0, 0, 0);
...
//  std::cout << "DST: " << dst <<"\n"
//          << "m_ipv4->GetAddress (1, 0): " << m_ipv4->GetAddress (1, 0).GetBroadcast () <<"\n"
//          << "DstPos: " << dstPos << "\n"
//          << "Invalidpos: " << m_locationService->GetInvalidPosition () << "\n";
//  
//  std::cout << "caldistance: " << CalculateDistance (dstPos, m_locationService->GetInvalidPosition ()) <<"\n";
...
//  if (CalculateDistance (dstPos, m_locationService->GetInvalidPosition ()) == 0 && m_locationService->IsInSearch (dst))
  ...
  if(m_neighbors.isNeighbour (dst))
    {
      nextHop = dst;
//      if (p->GetSize() > 1000) std::cout << "Route Output| Next hop is the destination!\n";
    }
  else
    {
//      if (p->GetSize() > 1000) 
//      {
//        std::cout << "~~~~~~~~~~~~~~~~~~\nThe mechanism.."
//                  << "Route Output| dstPos " << dstPos << " MyPos: " << myPos << "\n";  
//      }
//      std::cout << "DstPos" << dstPos << "\n";
      nextHop = m_neighbors.BestNeighbor (dstPos, dstVel, myPos, myVel);      
//      if (p->GetSize() > 1000) std::cout << "Route Output| Next hop is " << nextHop << "\n";
//      std::cout << "---\n RouteOutput Node: [" << m_ipv4->GetObject<Node> ()->GetId () << "] -> NextHop: " << nextHop 
//              << " Packet size: " << p->GetSize() << "---\n ";
    }
...
//      std::cout << "Node: " << m_ipv4->GetObject<Node> ()->GetId () << " Exist route to " << route->GetDestination () << " from interface " << route->GetSource ()<< "gateway " << route->GetGateway() << ".\n";
      if (oif != 0 && route->GetOutputDevice () != oif)
        {
//          std::cout << "Output device doesn't match. Dropped.\n";
...
//    if (m_ipv4)
//    {    
//        m_neighbors.PrintNeighbors(stream, m_ipv4->GetObject<Node> ()->GetId ())
//    }
  std::ostream* os = stream->GetStream ();
  *os << "Node: " << m_ipv4->GetObject<Node> ()->GetId ()
          << " Pos: " << m_ipv4->GetObject<MobilityModel> ()->GetPosition()
          << " Vel: " << m_ipv4->GetObject<MobilityModel> ()->GetVelocity() << "\n";
  m_neighbors.PrintPositionTable(stream);  
}
