...
const uint32_t veh_num = 125; // num of vehicles
/* Log files */
...
std::string TraceFile = "cvanet.tr";
std::string NetAnimFile = "cvanet.xml";
...
//                    " to " << STANodes.Get (sink)->GetId () << " with " << m_protocolName << "...");
        ...
        case 4:
        {
            ...
            serverApps.Add (Server.Install (STANodes.Get (62)));
            serverApps.Add (Server.Install (STANodes.Get (44)));
//            serverApps.Add (Server.Install (STANodes.Get (47)));
//            serverApps.Add (Server.Install (STANodes.Get (26)));
//            serverApps.Add (Server.Install (STANodes.Get (10)));
//            serverApps.Add (Server.Install (STANodes.Get (27)));
//            serverApps.Add (Server.Install (STANodes.Get (1))); 
            
//            serverApps.Get(3)->SetStartTime(Seconds(94));
//            serverApps.Get(4)->SetStartTime(Seconds(82));
            
            serverApps.Start (Seconds (startTime));
            serverApps.Stop (Seconds (endTime));
            
            serverApps.Get(0)->SetStartTime(Seconds(62));
            serverApps.Get(0)->SetStopTime(Seconds(endTime));

            serverApps.Get(1)->SetStartTime(Seconds(66));
            serverApps.Get(1)->SetStopTime(Seconds(endTime));
//            
//            serverApps.Get(2)->SetStartTime(Seconds(200));
//            serverApps.Get(2)->SetStopTime(Seconds(endTime));

            
            UdpClientHelper Client0 (STAIfaceContainer.GetAddress (62), 9);
            UdpClientHelper Client1 (STAIfaceContainer.GetAddress (44), 9);
//            UdpClientHelper Client2 (STAIfaceContainer.GetAddress (47), 9);
//            UdpClientHelper Client4 (STAIfaceContainer.GetAddress (27), 9);
//            
...
//            Client4.SetAttribute ("PacketSize", UintegerValue (packetSize));
            ...
            clientApps.Add(Client1.Install (STANodes.Get (10)));
//            clientApps.Add(Client2.Install (STANodes.Get (19)));
//            clientApps.Add(Client4.Install (STANodes.Get (47)));

//            clientApps.Get(2)->SetStartTime(Seconds(86));
//            clientApps.Get(3)->SetStartTime(Seconds(95));
//            clientApps.Get(4)->SetStartTime(Seconds(83));
//            
            clientApps.Start (Seconds (startTime+1.0));
            clientApps.Stop (Seconds (endTime));
//        84
            clientApps.Get(0)->SetStartTime(Seconds(63));
            clientApps.Get(0)->SetStopTime(Seconds(endTime));
            //            127
            clientApps.Get(1)->SetStartTime(Seconds(67));
            clientApps.Get(1)->SetStopTime(Seconds(endTime));
//            
//            clientApps.Get(2)->SetStartTime(Seconds(200));
...
{
  std::ofstream logs;
  logs.open ((outdir + log_1).c_str(),std::ios::app);
  for (uint32_t i = 0; i < STANodesNum; ++i) {
...
VanetSims::installGpsr_IfChosen()
{
    if (m_protocol == 5)
    {
        ...
        Ptr<OutputStreamWrapper> routingStream = Create<OutputStreamWrapper> ("table.txt", std::ios::out);
        gpsr.PrintRoutingTableAt(Seconds(66), STANodes.Get(41), routingStream);        
        gpsr.PrintRoutingTableAt(Seconds(66), STANodes.Get(99), routingStream);
        
        gpsr.PrintRoutingTableAt(Seconds(69), STANodes.Get(10), routingStream);        
        gpsr.PrintRoutingTableAt(Seconds(69), STANodes.Get(44), routingStream);
